	...
	for _, r := range results {
		if r.Valid {
			...
			if r.ExitIP != "" && r.ExitLocation != "" {
				if err := c.storage.UpdateProxyExitInfo(r.Proxy.ID, r.ExitIP, r.ExitLocation, latencyMs, r.Risk.IPAPIIsScore, r.Risk.Flags, r.Risk.CFBlocked, r.Risk.AIReachability); err != nil {
