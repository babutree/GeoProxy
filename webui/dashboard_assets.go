...
// 会话出口品质档：与卫星轨道 orbitQualityTrack 同一套阈值，供连线按品质匹配。
function orbitSessionQualityTrack(s){const g=String((s&&s.quality_grade)||'').trim().toUpperCase();if(g==='S'||g==='A'||g==='B'||g==='C'||g==='D')return g.toLowerCase();const lat=Number(s&&s.latency||0);if(lat>0&&lat<=200)return 's';if(lat>0&&lat<=500)return 'a';if(lat>0&&lat<=1000)return 'b';if(lat>0&&lat<=2000)return 'c';if(lat>0)return 'd';return ''}
