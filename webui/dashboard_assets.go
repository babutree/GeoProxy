...
// editNote: 来源无关的备注编辑（订阅节点也可用）。以 proxy-id 为主身份提交至 /api/manual-node/note。
async function editNote(id){return runAsync('备注更新失败',async()=>{const current=allProxies.find(p=>Number(p.id)===Number(id))||{};const note=prompt('备注',current.note||'');if(note===null)return;await api('/api/manual-node/note',{method:'POST',body:JSON.stringify({id:Number(id),note})});await loadProxies();showToast('备注已更新')})}
...
function proxyRowHTML(p){const addr=addressArg(p.address);const id=proxyIDArg(p);const manual=p.source==='manual';const st=nodeState(p);const label=html(nodeLabel(p));const showRegion=isAvailable(p)&&isKnownRegion(p);const toggleBtn=(st==='paused')?'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),true)">启用</button>':'<button class="mini" onclick="toggleProxy('+id+',decodeURIComponent(\''+addr+'\'),false)">停用</button>';const testBtn='<button class="mini" onclick="testProxy('+id+',decodeURIComponent(\''+addr+'\'))">测试</button>';const copyBtn='<button class="mini" onclick="copyProxyCred('+id+')">复制</button>';const baseActions=testBtn+' '+copyBtn+' '+toggleBtn;const manageBtn=manual?('<button class="mini" onclick="manageManualNode('+id+',decodeURIComponent(\''+addr+'\'))">管理</button>'):'';const actions=baseActions+(manageBtn?(' '+manageBtn):'');const latencyText=Number(p.latency)>0?html(p.latency)+' ms':'--';const sel=manual?'<input type="checkbox" class="proxy-select" value="'+id+'">':'';const nameCell='<td class="note-edit" title="'+label+' · 点击编辑备注" style="cursor:pointer" onclick="editNote('+id+')">'+label+'</td>';return '<tr><td>'+sel+'</td><td>'+starBtn(p)+'</td>'+nameCell+'<td>'+protocolBadges(p)+'</td><td>'+(showRegion?'<span class="badge ok">'+html(regionOf(p)).toUpperCase()+'</span>':'<span class="muted">--</span>')+'</td><td class="mono">'+html(p.exit_ip)+'</td><td>'+latencyText+'</td><td>'+abuserBadge(p.ipapiis_score)+'</td><td>'+ipapiFlagsBadges(p.ipapi_flags,!!p.ipapi_flags_seen)+'</td><td>'+cfBadge(p.cf_blocked)+'</td><td>'+aiBadges(p.ai_reachability)+'</td><td>'+html(sourceLabel(p))+'</td><td>'+stateBadge(st)+'</td><td><span class="ops">'+actions+'</span></td></tr>'}
