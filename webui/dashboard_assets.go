...
// jsArg: 把值安全嵌入 HTML 属性里的 JS 表达式上下文（如 onclick="f(...)"）。
// html() 只做 HTML 文本转义，用在 JS 字符串上下文是错的：浏览器会先把 &#39; 解码回
// 单引号再交给 JS 解析器，转义等于无效。这里用 JSON.stringify 生成 JS 字面量，
// 再对 & < > " ' 做 HTML 属性转义，两层上下文都正确。
function jsArg(value){return html(JSON.stringify(value===undefined||value===null?'':String(value)))}
...
// sessionRegionKey 取会话的地域聚合键，必须与卫星/地域面板的分桶键 regionOf(p) 同源。
// 优先用绑定节点当前的 region：卫星与地域面板都按 regionOf(p) 分桶，直接读节点可
// 保证会话一定落在真实存在的桶上（节点地域被改写后也不会漂移）。
// 其次用 selected_region：selector.Resolve 用 proxy.Region 写入 affinity 绑定，
// 经 /api/sessions 下发为 selected_region，与 regionOf(p) 同源。
// 最后兜底 exit_region（实测 exit_location 推导，可能与节点登记地域不同）。
function sessionRegionKey(s){if(s&&Number(s.proxy_id)>0){const p=allProxies.find(function(x){return Number(x.id)===Number(s.proxy_id)});if(p&&isKnownRegion(p))return regionOf(p)}const sel=String((s&&s.selected_region)||'').trim().toLowerCase();if(sel&&sel!=='unknown')return sel;const exit=String((s&&s.exit_region)||'').trim().toLowerCase();if(exit&&exit!=='unknown')return exit;return ''}
// 地域聚合：按可用节点数倒序；总览最多 TopN，不足则全显；全量页展示全部。
function buildRegionStats(){const map={};allProxies.filter(p=>isAvailable(p)&&isKnownRegion(p)).forEach(p=>{const r=regionOf(p);if(!map[r])map[r]={region:r,count:0,latSum:0,latN:0,s:0,a:0,b:0,c:0,d:0,sess:0};const st=map[r];st.count++;const lat=Number(p.latency||0);if(lat>0){st.latSum+=lat;st.latN++}const g=qualityOf(p);if(g==='S')st.s++;else if(g==='A')st.a++;else if(g==='B')st.b++;else if(g==='C')st.c++;else if(g==='D')st.d++});const sessByR={};(Array.isArray(orbitSessions)?orbitSessions:[]).forEach(s=>{const r=sessionRegionKey(s);if(!r)return;sessByR[r]=(sessByR[r]||0)+1});Object.keys(map).forEach(function(r){map[r].sess=sessByR[r]||0;map[r].avgLat=map[r].latN?Math.round(map[r].latSum/map[r].latN):0});return Object.keys(map).map(function(k){return map[k]}).sort(function(a,b){return b.count-a.count||a.region.localeCompare(b.region)})}
...
// deleteSub: /api/subscription/delete 存在「订阅已删但受管文件清理失败」的 500 分支，
// 此时服务端状态已变。无论成功或该类部分失败都必须重新拉取列表，
// 否则页面会继续显示已被删除的订阅（表现为「确认后仍未删除」）。
async function deleteSub(id){return runAsync(t('err_delete'),async()=>{if(!(await showConfirm(t('confirm_delete_sub'))))return;try{await api('/api/subscription/delete',{method:'POST',body:JSON.stringify({id})})}finally{await runAsync(t('err_refresh'),()=>Promise.all([loadSubscriptions(),loadStats(),loadProxies()]))}showToast(t('toast_sub_deleted'))})}
...
function renderAPIKeys(keys){const body=document.getElementById('apikey-rows');if(!body)return;const list=Array.isArray(keys)?keys:[];if(!list.length){body.innerHTML='<tr><td colspan="5" class="empty">'+html(t('no_apikeys'))+'</td></tr>';return}body.innerHTML=list.map(k=>{const id=jsArg(k.id);const name=html(k.name);const created=html(formatAPIKeyTime(k.created_at));const last=html(formatAPIKeyTime(k.last_used_at));const disabled=!!(k.disabled===true||Number(k.disabled)===1);const st=disabled?'<span class="badge warn">'+html(t('apikey_revoked'))+'</span>':'<span class="badge ok">'+html(t('apikey_active'))+'</span>';const revokeBtn=disabled?'':'<button class="mini" onclick="revokeAPIKey('+id+')">'+html(t('apikey_revoke'))+'</button> ';return '<tr><td>'+name+'</td><td>'+created+'</td><td>'+last+'</td><td>'+st+'</td><td>'+revokeBtn+'<button class="mini danger" onclick="deleteAPIKey('+id+')">'+html(t('apikey_delete'))+'</button></td></tr>'}).join('')}
