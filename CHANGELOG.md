...
- **WebUI 登录密码改用加盐 PBKDF2-HMAC-SHA256**（210k 迭代，随机 16 字节盐），编码为 `pbkdf2-sha256$<迭代数>$<盐>$<派生密钥>`；迭代数写进串里，将来调参不会让存量哈希失效。存量无盐 SHA-256 **不会被锁在外面**：仍可正常登录，并在登录成功的瞬间（唯一同时拿得到明文与旧哈希的时机）透明重写为 PBKDF2。校验失败绝不触发升级——否则一次错误猜测就会改写已存储的凭据；落盘失败也不阻断登录，且运行态保留旧哈希以免内存与磁盘分裂。
  **代理认证密码（`ProxyAuthPasswordHash`）刻意保持无盐 SHA-256**，这不是漏改：一是它在每个 HTTP 请求 / 每条 SOCKS5 连接上校验，PBKDF2 实测 95ms/次，100 req/s 就要约 10 个 CPU 核，网关会直接瘫痪；二是代理密码的**明文**按设计与哈希同存于 `config.json`（供 WebUI 复制含密码的完整代理 URL），能读到哈希的攻击者同时就读到了明文，强化哈希保护不了任何东西。两者安全模型本就不同——登录密码是纯哈希凭据，代理密码是明文凭据。登录侧的 95ms 配合既有失败锁定（5 次失败锁 1 分钟）单客户端最多消耗 474ms CPU，DoS 面可控
